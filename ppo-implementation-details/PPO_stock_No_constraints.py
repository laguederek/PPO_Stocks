...
class Agent(nn.Module):
    ...
    def apply_cardinality_constraints(self, action):
        ...
        for act in action:
            ...
            new_act = new_act / (new_act.sum() + 1e-8)
...
if __name__ == "__main__":
    ...
    max_weight = 1.0
    agent = Agent(envs, max_cardinality=max_cardinality,min_cardinality=min_cardinality, min_weight=min_weight, max_weight=max_weight).to(device)
    # print(agent)
    ...
    # Save model
    # torch.save(agent.state_dict(), "ppo_portfolio_agent_stocks_no_contraints.pt")

    # Recreate environment for evaluation
    eval_env = PortfolioEnv(price_data=price_data)
    agent.load_state_dict(torch.load("ppo_portfolio_agent_stocks_no_contraints.pt", map_location=device))
    ...
    plt.plot(ppo_portfolio_values, label="Trained PPO Portfolio no Constraints", linewidth=2)
    ...
    results = pd.DataFrame({
        "Portfolio": ["PPO Agent no contraints train", "Fixed 2%"],
    ...
    # Plot evolution of portfolio values
    plt.figure(figsize=(12, 6))
    plt.plot(ppo_portfolio_values, label="Trained PPO Portfolio no Constraints test", linewidth=2)
    plt.plot(fixed_portfolio_values, label="Fixed 2% Portfolio", linestyle="--", linewidth=2)
    ...
    # Create results table
    results = pd.DataFrame({
        "Portfolio": ["PPO Agent no constraints test", "Fixed 2%"],
        "Return": [ppo_ret, fixed_ret],
        "Volatility": [ppo_vol, fixed_vol],
        "Sharpe Ratio": [ppo_sharpe, fixed_sharpe]
    })
